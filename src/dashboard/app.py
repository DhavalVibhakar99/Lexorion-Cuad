    ...
    if contract_text and st.button("🔍 Analyze Contract", type="primary"):
        with st.spinner("Analyzing contract..."):
            ...
            try:
                ...
                from src.models.hybrid_pipeline import HybridPipeline, print_risk_profile
                ...
                if not paragraphs:
                    ...
                    col1.metric("Paragraphs Analyzed", profile.total_paragraphs)
                    col2.metric("Clauses Flagged", profile.flagged_paragraphs)
                    col3.metric("Processing Time", f"{profile.processing_time_seconds:.1f}s")
                    
                    # Risk heatmap
                    st.plotly_chart(
                        render_risk_heatmap(profile.risk_scores),
                    ...
                    if flagged:
                        ...
                        for d in flagged:
                            with st.expander(
                                f"{'🔴' if d.risk_level in ['high', 'critical'] else '🟡'} "
                                f"{d.risk_category.replace('_', ' ').title()} "
                                f"(confidence: {d.confidence:.0%})"
                            ):
                                st.markdown(f"**Risk Level:** {d.risk_level}")
                                st.markdown(f"**Summary:** {d.summary}")
                                st.markdown(f"**Model:** {d.model_used}")
        ...
        st.plotly_chart(render_risk_heatmap(sample_scores), use_container_width=True)
